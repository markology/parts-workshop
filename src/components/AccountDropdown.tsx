...
export default function AccountDropdown({
  ...
  useEffect(() => {
    if (dropdownOpen) {
      const triggerElement = triggerRef?.current || containerRef?.current || internalTriggerRef.current;
      if (triggerElement) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const dropdownMenu = (dropdownRef.current as { dropdownMenu?: HTMLElement }).dropdownMenu;
  ...
  const defaultMenuClassName = useWorkspaceTheme
    ? "fixed rounded-lg shadow-lg z-[100]"
        ...
        >
          ...
          {showDashboard && (
            <button
              ...
              onMouseLeave={useWorkspaceTheme ? (e) => {
